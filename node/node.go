...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, ancient string, namespace string, readonly, pruneAncientData bool) (ethdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
	...
	var err error
	if n.config.DataDir == "" {
		db, err = rawdb.NewDatabaseWithFreezer(memorydb.New(), "", namespace, readonly, pruneAncientData)
	} else {
		db, err = openDatabase(openOptions{
			Type:              n.config.DBEngine,
			Directory:         n.ResolvePath(name),
